...
r = zsc(33)/zsc(36);%compute the laser power/laser reerence ratio (to adjust for drift in laser output power over time in mW/mW) = experimentally measured in clear water
...
if nargin ~=4
    for i = 1:row
        scat(i,:)=data(i,1:32)/tau(i)-zsc(1:32)*data(i,36)/zsc(36); %correct measured intensity for 1) fluctuations in beam power, 2)background scatter into rings by seawater
    end
else
