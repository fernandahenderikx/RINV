...
% THIS IS A MODIFICATION OF THE ORIGINAL FASTMIE.M TO INCLUDE ACCEPTANCE
% ANGLE CALCULATIONS THAT MATCH THE C-STAR TRANSMISSOMETER, AND CALCULATION
% OF THE Q11 MUELLER MATRIX FOR VSF APPLICATIONS
...
% 31 May 2022 Added in acceptance angle calculations for C-Star instrument (James Allen, Un. of Hawaii)

% 10 Oct 2022 Added in P11 for VSF (James Allen, Un. of Hawaii)


function [s1, s2, Q_scat, Q_ext, Q_back, Q_extStar, Q11] = fastmie_mod(x,m,nang,varargin)
...
if isempty(nang)
    theta = varargin{1}(:)'; % Make sure this is a row
else
...
% Calculate Q11 of Mueller Matrix (the efficiency of the p11 matrix)
Q11        = 0.5*(abs(s1).^2 + abs(s2).^2)';
ang        = theta;
angDeg     = ang/pi*180;
ntgrand    = Q11.*sin(ang);
Q11IntFull = trapz(angDeg,ntgrand,2);
indAcc     = find(angDeg >= 1.2);
Q11IntAcc  = trapz(angDeg(indAcc),ntgrand(indAcc),2);
Q_extStar  = Q_ext.*(Q11IntAcc/Q11IntFull);
