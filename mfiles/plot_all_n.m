% plot all n_ideals 
close all;clearvars -except mdir procdata figsdir datadir; 
mdir=cd;
cd ..
cd data
datadir = cd;
...
cd(procdata)
filelist = dir('n_slope*'); % everytime you run additional analysis in MAIN.m (additional "runX.m" outputs) the order of these may change. Be careful when calling the appropriate file below.
...
figure_vertical
    ...
    f=1;
for k = [1 11 9 10] % make sure fig text(...) matches these numbers.
    load(filelist(k).name)
axes(ha(f))
a = nan(18,24);
lt = gmt-10/24;
...
text(3,1.01955,['avg  = ' avgn{11,:} '{\pm}' stdevn{11,:}]);
text(3,1.01855,['{\Delta}  = ' deltaavgn{11,:} '{\pm}' deltastdevn{11,:} '%']);
axes(ha(3))
ylabel('Real Index of Refraction [{\it{n}}]')
set(gca,'XTick',[3 6 9 12 15 18 21],'XTickLabel',[3 6 9 12 15 18 21])
addlabel('c)')
...
text(3,1.0395,['avg  = ' avgn{9,:} '{\pm}' stdevn{9,:}]);
text(3,1.0375,['{\Delta}  = ' deltaavgn{9,:} '{\pm}' deltastdevn{9,:} '%']);
axes(ha(4))
set(gca,'XTick',[3 6 9 12 15 18 21],'XTickLabel',[3 6 9 12 15 18 21])
%set(gca,'YTickLabel','')
addlabel('d)')
...
text(3,1.031,['avg  = ' avgn{10,:} '{\pm}' stdevn{10,:}]);
text(3,1.0295,['{\Delta}  = ' deltaavgn{10,:} '{\pm}' deltastdevn{10,:} '%']);
xlabel('Local Hour');
cd(figsdir)
...
%% sensitivity analysis - truncate data at 10, 20, 50 um
  figure_vertical
    ...
    f=1;
for k = [5 6 7] % make sure these numbers match the numbers called in text(...) below
    load(filelist(k).name)
axes(ha(f))
...
text(3,1.033,['avg  = ' avgn{5,:} '{\pm}' stdevn{5,:}]);
text(3,1.032,['{\Delta  = ' deltaavgn{5,:} '{\pm}' deltastdevn{5,:} '%}']);
ylabel('Real Index of Refraction [{\it{n}}]')
axes(ha(2))
set(gca,'XTick',[3 6 9 12 15 18 21],'XTickLabel','')
set(gca,'YTickLabel','')
...
text(3,1.032,['avg  = ' avgn{6,:} '{\pm}' stdevn{6,:}]);
text(3,1.031,['{\Delta}  = ' deltaavgn{6,:} '{\pm}' deltastdevn{6,:} '%']);
axes(ha(3))
ylabel('Real Index of Refraction [{\it{n}}]')
set(gca,'XTick',[3 6 9 12 15 18 21],'XTickLabel',[3 6 9 12 15 18 21])
addlabel('c)')
...
text(3,1.033,['avg  = ' avgn{7,:} '{\pm}' stdevn{7,:}]);
text(3,1.032,['{\Delta}  = ' deltaavgn{7,:} '{\pm}' deltastdevn{7,:} '%']);
cd(figsdir)
