...
% 4um size class.
N02_4 = No.*((dias(1:18)./dias(refdiam)).^-slope);
N4_6 = data(:,refdiam).*((dias([19:refdiam-1])./dias(refdiam)).^-slope);

% add those extrapolated portions to the data to make a full psd
fullpsdn = data;
fullpsdn(:,[1:18])=repmat(N02_4,length(fullpsdn(:,1)),1);
fullpsdn(:,[19:refdiam-1])=N4_6;
